...
const nameIsFree = (req: Request, res: Response, next: NextFunction) => {
  void (async () => {
    ...
    if (itemWithSameName && itemWithSameName.id_item !== item.id_item) {
...
const create = (newItem: IItem): Promise<number> => {
  return connection
    .promise()
    .query<ResultSetHeader>(
      'INSERT INTO items (name, id_category, id_size_type) VALUES (?, ?, ?)',
      [newItem.name, newItem.id_category, newItem.id_size_type]
    )
